...
async def get_quote(ticker):
    ...
    yticker = "BTC-USD" if ticker=="BTC" else ticker
    data = await fetch_yahoo(yticker,int(now)-86400*3,int(now))
    if data:
        try:
            ...
            if price:
                result={"price":round(float(price),2),"change_pct":round((price-prev)/prev*100,2) if prev else 0,"source":"live"}
                _quote_cache[ticker]=result; _cache_time[ticker]=now
                return result
        except: pass
    d=DEFAULT_PRICES.get(ticker,{"price":100.0,"change_pct":0.0})
...
async def get_all_quotes(tickers):
    ...
    return {tickers[i]:(r if not isinstance(r,Exception) else DEFAULT_PRICES.get(tickers[i],{"price":100,"change_pct":0})) for i,r in enumerate(results)}
...
def compute_signal(ticker,price,avg_cost,change_pct):
    f=FUND.get(ticker,{}); pnl=((price-avg_cost)/avg_cost*100) if avg_cost else 0
    score,reasons=0,[]
    if f.get("peg") and f["peg"]<1.0: score+=2; reasons.append("PEG < 1 — undervalued growth")
    elif f.get("peg") and f["peg"]<1.5: score+=1; reasons.append("PEG attractive")
...
async def build_snapshot():
    ...
    for h in hs:
        ...
        price=q.get("price",h["avg_cost"]); value=price*h["shares"]
        ...
        sig=compute_signal(h["ticker"],price,h["avg_cost"],q.get("change_pct",0))
...
async def call_claude(prompt,max_tokens=1024):
...
async def api_delete_holding(hid: int):
    _store["holdings"]=[h for h in holdings() if h["id"]!=hid]; return {"ok":True}
...
async def api_delete_watchlist(wid: int):
    _store["watchlist"]=[w for w in watchlist() if w["id"]!=wid]; return {"ok":True}
...
const F = {
  ...
  HIMS:{pe:45,ps:4.2,peg:0.9,dilution:"~2.7%/yr",sector:"Digital Health"},
  LLY:{pe:52,ps:14,peg:1.4,dilution:"~-1%/yr",sector:"Pharma/GLP-1"},
  OSCR:{pe:12,ps:0.8,peg:0.4,dilution:"~4.6%/yr",sector:"Health Insurance"},
...
function editH(id){
  ...
  document.getElementById(`hr${id}`).innerHTML=`
    ...
    <td><div style="display:flex;gap:5px;justify-content:flex-end;">
      ...
      <button class="bts" onclick="renderHoldings()">CANCEL</button>
    </div></td>`;
}
